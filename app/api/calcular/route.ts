 ...
 * Recibe los datos del viaje y devuelve el cálculo de la IA.
 * También guarda el viaje en la base de datos.
 */
...
export async function POST(request: Request) {
  try {
    ...
    // Validaciones
    if (!body.camion_id) {
      return NextResponse.json({ error: 'Seleccioná un camión' }, { status: 400 });
    }
    if (!body.kilometros || body.kilometros <= 0) {
      return NextResponse.json({ error: 'Ingresá los kilómetros del viaje' }, { status: 400 });
    }
    if (body.peso_carga === undefined || body.peso_carga < 0) {
      return NextResponse.json({ error: 'Ingresá el peso de la carga' }, { status: 400 });
    }
    if (!body.precio_combustible || body.precio_combustible <= 0) {
      return NextResponse.json({ error: 'Ingresá el precio del combustible' }, { status: 400 });
    }
    ...
    const { data: viaje, error: viajeError } = await supabase
      ...
      .insert({
        ...
        // Flete (opcional, para calcular rentabilidad)
    ...
    if (viajeError) {
      ...
      // Igual devolvemos el cálculo aunque no se haya guardado
    }
