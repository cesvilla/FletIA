 ...
 * Maneja el estado del sidebar móvil y el logout.
 */
export default function DashboardClient({ email, empresa }: DashboardClientProps) {
  ...
  return (
    <div className="flex min-h-screen bg-bg">
      ...
      <aside
        className={`
          fixed top-0 left-0 h-screen w-[220px] bg-ink flex flex-col z-50
      ...
      >
        ...
        <div className="p-6 border-b border-white/10 flex items-start justify-between">
          <div>
            ...
            <div className="font-mono text-[8px] tracking-[2px] text-white/30 mt-1 uppercase">
        ...
        <nav className="flex-1 py-4 overflow-y-auto">
          <div className="font-mono text-[8px] tracking-[2px] text-white/25 px-5 my-4 uppercase">
            Principal
          </div>
          <a className="flex items-center gap-2.5 px-5 py-2.5 text-white text-sm font-medium bg-accent/15 border-l-2 border-accent">
        ...
        <div className="p-5 border-t border-white/10">
          <div className="flex items-center gap-3 mb-3">
            <div className="w-8 h-8 bg-accent rounded-full flex items-center justify-center font-bold text-xs text-white flex-shrink-0">
              {iniciales}
            </div>
          ...
          <button
            ...
            className="w-full text-left font-mono text-[10px] text-white/40 hover:text-accent transition-colors"
          >
            → Cerrar sesión
          </button>
        </div>
      ...
      <main className="flex-1 md:ml-[220px]">
        ...
        <div className="bg-card border-b border-ink/10 px-4 md:px-7 h-14 flex items-center justify-between sticky top-0 z-30">
          <div className="flex items-center gap-3">
            ...
            <div className="hidden md:block font-mono text-[10px] text-ink-3 px-2 py-0.5 bg-surface rounded-full">
              Sprint 1 · MVP
            </div>
          </div>
          <button className="bg-accent text-white px-4 py-2 text-xs font-bold hover:bg-accent/90 transition-colors">
            ⚡ Calcular viaje
          </button>
        </div>
        ...
        <div className="p-4 md:p-7 max-w-6xl">
          ...
          <div className="bg-card border border-ink/10 p-6 md:p-8 mb-6 animate-fade-up">
            <div className="font-mono text-[10px] tracking-[3px] text-accent uppercase mb-3">
              // Bienvenido a FletIA
            </div>
            <h1 className="font-display text-4xl md:text-5xl font-black tracking-tight mb-3 leading-none">
              Hola, <span className="text-accent">{empresa}</span> 👋
            </h1>
            <p className="text-ink-2 text-base mb-6 max-w-2xl">
              Tu cuenta está lista. Para empezar a calcular el costo exacto de tus viajes, primero necesitamos registrar tus camiones.
            </p>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 max-w-xl">
              <button className="bg-ink text-white px-5 py-3 text-sm font-bold hover:bg-ink-2 transition-colors text-left">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-6">
            {[
              ...
              <div
                key={i}
                className="bg-card border border-ink/10 p-4 md:p-5 animate-fade-up"
                style={{ animationDelay: `${0.1 + i * 0.05}s` }}
              >
                <div className="font-mono text-[9px] tracking-[2px] text-ink-3 uppercase mb-2">
                  {kpi.label}
                </div>
                <div className="font-display text-2xl md:text-3xl font-bold leading-none mb-1.5 text-ink">
                  {kpi.value}
                </div>
          ...
          <div className="bg-card border border-ink/10 p-6 animate-fade-up">
            <div className="flex items-center gap-2 mb-4">
              <div className="font-mono text-[10px] tracking-[3px] text-success uppercase">
                ✓ Sprint 1 completado
              </div>
            </div>
            ...
            <div className="mt-5 pt-5 border-t border-ink/10">
              <div className="font-mono text-[10px] text-ink-3 tracking-wider mb-2">
                // PRÓXIMO PASO
              </div>
              <div className="text-sm text-ink-2">
                Cuando tengas esto corriendo y registrado tu cuenta, avisame y arrancamos el Sprint 2: <strong className="text-ink">alta de camión funcional + tabla de tu flota</strong>.
              </div>
            </div>
          </div>
